    ...
    public class FoundationManager : MonoBehaviour
    {
        ...
        public void LoadBaseFloors()
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                if (Physics.Raycast(leftRay, out hit, 1f, LayerMask.GetMask("OfficeDoor")) 
                    || Physics.Raycast(rightRay, out hit, 1f, LayerMask.GetMask("OfficeDoor"))
                    || Physics.Raycast(upRay, out hit, 1f, LayerMask.GetMask("OfficeDoor"))
                    || Physics.Raycast(downRay, out hit, 1f, LayerMask.GetMask("OfficeDoor")))
                {
                ...
                // Detects wallCorners.
                if (Physics.Raycast(leftUpRay, out hit, 1.5f, LayerMask.GetMask("Wall")) && hit.collider.gameObject.name.Contains("WallCorner"))
                {
                    f.OriginalLeftUpWallCorner = f.LeftUpOutWallCorner = hit.collider.gameObject;
                }

                if (Physics.Raycast(leftDownRay, out hit, 1.5f, LayerMask.GetMask("Wall")) && hit.collider.gameObject.name.Contains("WallCorner"))
                {
                    f.OriginalLeftDownWallCorner = f.LeftDownOutWallCorner = hit.collider.gameObject;
                }

                if (Physics.Raycast(rightUpRay, out hit, 1.5f, LayerMask.GetMask("Wall")) && hit.collider.gameObject.name.Contains("WallCorner"))
                {
                    f.OriginalRightUpWallCorner = f.RightUpOutWallCorner = hit.collider.gameObject;
                }

                if (Physics.Raycast(rightDownRay, out hit, 1.5f, LayerMask.GetMask("Wall")) && hit.collider.gameObject.name.Contains("WallCorner"))
                {
